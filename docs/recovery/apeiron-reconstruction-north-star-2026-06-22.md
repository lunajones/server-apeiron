...
| Damage / defense / posture / stamina | `PARTIAL` | DB has skill damage/posture, combat core, defense contracts; server has `internal/gameapi/impact_pipeline_bridge.go`, temporal impact tests, and creature temporal skill impact tests. `gameapi` now sends runtime impacts through `internal/combat.ImpactResolutionPipeline`, including block/parry/iframe decisions. | Full shared tick-by-tick damage scheduler is still missing. The recovered runtime defense contract still uses a local reconstructed guard profile until DB/gRPC defense contract loading is wired into `gameapi`. | Move recovered guard/core values to DB-loaded combat contracts, then split only where useful so stamina/defense/damage/action state have one authoritative runtime owner. |
