...
| Creature brain package | `PARTIAL` | `internal/ai` owns skill binding ranking, setup-policy movement tactics, cooldown unavailability, resource budget, repeat-skill penalty, orbit side memory, and active-skill setup/continuation. `gameapi` passes DB-loaded stamina/repeat/setup policy and skill costs into the brain and spends creature stamina only when a skill starts. | Rich perception/threat inputs and creature action-runtime parity are still incomplete. Wolf grounding/visual plane still needs Unreal/runtime validation. | Continue from this module, not from `gameapi` branches: add perception/threat inputs and creature action-runtime parity before calling creature runtime fully restored. |
...
1. `P0`: Restore creature brain architecture in `server-apeiron/internal/ai`.
   ...
   - Remaining: richer perception/threat inputs and creature action runtime parity.
