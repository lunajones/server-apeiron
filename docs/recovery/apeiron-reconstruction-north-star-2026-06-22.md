...
| Skill movement: player F/R/M1 | `PARTIAL` | DB has movement bindings; server combat has `player_skill_combat_system.go` and `skill_movement_helpers.go`; Unreal bridge maps `SkillGroundedAction` and `PostActionGrounded`. Server guard tests now prove M1-1/M1-2/M1-3/R/F keep owned root during skill, lateral sprint input cannot apply normal movement during owned root, and post-action returns sprint-strafe after handoff. | History shows this regressed repeatedly. Unreal/PIE automation still must prove the same against real client prediction and aggressive yaw. | Extend movement regression suite for stationary M1, M1 chain, F/R, post-action sprint/strafe, and aggressive yaw. Fix common owner, not per-skill patches. |
...
| Temporal melee hitboxes | `PARTIAL` | DB has temporal hitbox tables/seeds; server has `internal/hitbox` runtime/tests, and `gameapi` now evaluates enabled motion profiles through `impact_temporal.go` instead of expanding every sample into one static full-area hit. Player M1/R/F and wolf bite/lunge/maul recovery fixtures mirror DB motion profile IDs and sample geometry. Strict runtime coverage now rejects any damaging required skill that lacks temporal hitbox/motion profile data. `gameapi` now routes player and creature impact resolution through a pending, instance-keyed temporal impact runner. `SubmitCommand` starts the player action; damage is applied by the runner when the hitbox window is reached. Creature skills use the same enqueue/runner path. | The runner is still in `gameapi` and not yet split into a dedicated combat runtime package. PIE/debug visualization still needs proof. | Promote the runner behind an explicit damage-group runtime owner shared by player and creature, then remove/isolate static compatibility for canonical melee skills. |
| Damage / defense / posture / stamina | `PARTIAL` | DB has skill damage/posture, combat core, defense contracts; server has `internal/gameapi/impact_pipeline_bridge.go`, temporal impact tests, and creature temporal skill impact tests. `gameapi` now sends runtime impacts through `internal/combat.ImpactResolutionPipeline`, including block/parry/iframe decisions. `RuntimeContracts` now loads player/wolf combat core profiles and player/creature defense contracts from DB/profile gRPC, and strict coverage fails if those contracts are missing. Damaging skills must also carry max targets plus temporal damage group/motion samples. Required skill movement contracts must define start phase, handoff policy, normal-input policy, phase window policy, prediction policy, root owner, and contact policy. The pending impact runner now blocks duplicate damage and duplicate enqueue for the same action instance. | Recovery fixtures still exist for dev/test reconstruction only, but normal DB loading no longer invents local guard/core values. Damage-group orchestration still lives inside `gameapi`. | Keep stamina/defense/damage/action state under one authoritative runtime owner and split out only when the package boundary can preserve that ownership. |
...
| Creature brain package | `PARTIAL` | `internal/ai` owns skill binding ranking, setup-policy movement tactics, cooldown unavailability, resource budget, repeat-skill penalty, orbit side memory, and active-skill setup/continuation. `gameapi` passes DB-loaded stamina/repeat/setup policy and skill costs into the brain and spends creature stamina only when a creature action instance starts. Creature locomotion/AI publication and action lifecycle are now split out of `updateWolfPolicyLocked`. Creature skill root motion now owns position during the configured movement phase, while windup setup movement remains tactical/setup-owned. Creature action movement now has an explicit envelope for movement start/end, airborne window, landing inertia, passthrough, and stop-at-contact semantics. Orbit side switching now honors `side_flip_chance_multiplier` as deterministic policy chance instead of flipping every eligible window. | Rich perception/threat inputs and full queue/cancel/contact-control parity are still incomplete. Wolf grounding/visual plane still needs Unreal/runtime validation. | Continue from this module, not from `gameapi` branches: add perception/threat inputs and complete creature action-root runtime parity before calling creature runtime fully restored. |
...
| Creature skill movement | `PARTIAL` | DB has movement action contracts for wolf skills and legacy compatibility for lunge. Unreal has predictive creature skill visual motion. Server tests now prove creature temporal damage waits for hitbox window and resolves once per skill instance. Server tests also prove wolf lunge windup uses setup movement before root motion, active lunge uses skill root motion, creature action timing cannot complete before movement action completion, lunge allows pass-through by contract, and post-airborne landing inertia remains root-motion-owned. Wolf AI skill movement presentation now derives takeoff/landing/distance/duration from the movement contract instead of fixed numbers. | Target collision/carry/control response and visual grounding still need PIE validation. | gRPC-check lunge/maul contracts, add server tests for contact response/control interruption, then Unreal visual validation. |
...
| Recovery fixtures/fallbacks | `PARTIAL` | `RecoveryFixtureRuntimeContracts()` is now explicitly named/commented as a dev/test reconstruction fixture. `gameapi` command execution rejects dodge/leap/skill movement with missing contracts. Unreal rejects incomplete/fallback runtime reconciliation profiles for normal corrections. `RuntimeContracts.CoverageReport` now classifies readiness blockers by category instead of relying on one flat error string. | Fixture still exists for dev/test reconstruction, and other legacy DB compatibility surfaces still need consumer proof. | Keep fixture out of app boot. Continue classifying legacy surfaces as final authority, compat required, recovery-only, or dead. |
...
1. `P0`: Restore creature brain architecture in `server-apeiron/internal/ai`.
   ...
   - Remaining: richer perception/threat inputs and complete pass-through/contact/landing parity.
...
- `gameapi` now wraps creature skill execution in `actionruntime.Instance` with `ActorKindCreature`; `SkillRuntimeState.State` publishes action phase instead of the old action-name string, cooldown/stamina spend happen only on action start, and completed actions clear back to idle.
