...
| Recovery fixtures/fallbacks | `SUSPICIOUS` | `RecoveredRuntimeContracts()` remains in `internal/gameapi/contracts.go` and tests. Latest ledger says it is not normal boot path. | It can still mislead future work if treated as final. | Move recovery fixture intent into test-only naming or explicit dev fixture. Normal boot must fail with missing DB contracts. |
