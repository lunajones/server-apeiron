...
| Temporal melee hitboxes | `PARTIAL` | DB has temporal hitbox tables/seeds; server has `internal/hitbox` runtime/tests, and `gameapi` now evaluates enabled motion profiles through `impact_temporal.go` instead of expanding every sample into one static full-area hit. Player M1/R/F recovery fixture mirrors DB motion profile IDs and sample geometry. | Current recovered `gameapi` still applies impact as one runtime event rather than a full tick-by-tick damage group scheduler; wolf bite/lunge/maul need the same runtime proof. | Promote temporal impact into a full damage scheduler shared by player and creature, then remove/isolate static compatibility for canonical melee skills. |
...
- `bootstrap/015_temporal_hitbox_seed.sql` and recovered server fixtures use one coordinate convention: `offset_x` is forward and `offset_y` is lateral.
- Tests prove a future capsule/arc sample does not hit at the first active slice.
