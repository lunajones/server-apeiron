...
| Damage / defense / posture / stamina | `PARTIAL` | DB has skill damage/posture, combat core, defense contracts; server has `internal/gameapi/defense.go`, `impact.go`, temporal impact tests, and creature temporal skill impact tests. Wolf bite/lunge/maul recovered contracts carry damage/posture/hitbox data. | `internal/damage`, `internal/defense`, `internal/stamina` packages are absent. Logic is concentrated in `gameapi`; `defaultBlockArcDeg` exists. Creature impact now resolves once per runtime instance, but full shared damage scheduler is still missing. | Split only when useful: create authoritative combat-resolution modules or packages so DB contracts own values and runtime owns flow. Remove default gameplay numbers from normal paths. |
...
| Creature brain package | `PARTIAL` | `internal/ai` owns skill binding ranking, cooldown unavailability, resource budget, repeat-skill penalty, orbit side memory, and active-skill continuation. `gameapi` passes DB-loaded stamina/repeat policy and skill costs into the brain and spends creature stamina only when a skill starts. | Rich perception, setup memory, and creature action-runtime parity are still incomplete. Wolf grounding/visual plane still needs Unreal/runtime validation. | Continue from this module, not from `gameapi` branches: add perception/threat inputs, setup memory, and creature action-runtime parity before calling creature runtime fully restored. |
...
1. `P0`: Restore creature brain architecture in `server-apeiron/internal/ai`.
   ...
   - Remaining: richer perception, setup memory, and creature action runtime parity.
