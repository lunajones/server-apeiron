...
| Recovery fixtures/fallbacks | `SUSPICIOUS` | `RecoveredRuntimeContracts()` remains in `internal/gameapi/contracts.go` and tests. Latest ledger says it is not normal boot path. `gameapi` command execution no longer accepts dodge/leap/skill movement when their movement contracts are absent or invalid. | Recovery fixture can still mislead future work if treated as final. Unreal still has inert/default profile values that need explicit missing-profile visibility. | Move recovery fixture intent into test-only naming or explicit dev fixture. Normal boot must fail with missing DB contracts. |
