...
| DB migrations/protos/service surface | `PARTIAL` | DB has 34 create-only migrations, 20 bootstrap SQL files, generated Go, and proto services for profile/skill/creature/player/world/cache/inventory/observability. `RunMigrations` now resets the `apeiron` schema on app startup before executing the full baseline, matching the vertical-slice reset/seed workflow. `TestMigrationFilesAreFreshCreateOnlyBaseline` forbids `ALTER TABLE`/`ALTER COLUMN` in migrations. Server `gameapi` now has a runtime contract requirement manifest for required movement profile, base actions, M1/R/F, wolf skills, combat core, defense, weapon kit, and wolf brain policy. DB mirrors that manifest in `TestBootstrapSeedsMirrorServerRuntimeRequirementManifest` and `TestBootstrapSeedsMirrorRequiredSkillActionManifest`, proving bootstrap coverage for required runtime surfaces instead of relying on hollow compilation. | Current numbering is reconstructed/compact; original modern numbering from chats had later numbers. Unreal still needs equivalent consumer proof for every required field. The reset-on-start model is correct for the current dev slice, but it is not a production migration strategy. | Keep the create-only baseline strict while this vertical slice is reset/seed driven. Prove every Unreal/server-required field via proto, gRPC tests, and Unreal consumer validation. When persistence becomes real, create a separate production migration plan instead of reintroducing compatibility ALTERs into this baseline. |
...
| DB legacy compatibility | `PARTIAL` | `db-legacy-compatibility-audit-2026-06-22.md` classifies legacy surfaces. Recovery-only incremental compatibility migrations were retired from the live baseline; any still-required columns now live in canonical `CREATE TABLE` definitions. `gameapi` has a runtime surface classification table and `RuntimeStats` exposes `contracts.surface.*` statuses. Tests prove the normal server contract loader does not expose `GetSkillMovementEffect`. | Legacy DB endpoint still exists for compatibility and Unreal/external tool usage still needs the same consumer proof. Historical docs may still mention deleted compatibility migrations as recovery evidence, not runtime authority. | Keep `skill_movement_effect` as compat until Unreal/tool audit is complete, but do not allow it to become normal runtime authority. Do not add compatibility ALTER migrations back into the dev baseline. |
...
| Skill movement: player F/R/M1 | `PARTIAL` | DB has movement bindings; server combat has `player_skill_combat_system.go` and `skill_movement_helpers.go`; Unreal bridge maps `SkillGroundedAction` and `PostActionGrounded`. Server guard tests now prove M1-1/M1-2/M1-3/R/F keep owned root during skill, lateral sprint input cannot apply normal movement during owned root, stationary skills do not teleport on submit, mid-action movement progresses through snapshots, repeated sprint-forward M1/R/F returns to grounded move, and post-action returns sprint-strafe after handoff. 2026-06-23 follow-up restored Shield Rush as a longer committed run (`4.7m`, `830ms`, `430ms active`, `240ms recovery`) in DB seeds and server fixture contracts, and player forward-distance skill movement now commits movement target from horizontal player facing/aim before target entity or mouse position. 2026-06-23 follow-up resets command replay state on `AttachPlayer`, preventing a new PIE/client attach from having sequence `1` rejected against an old server-side sequence. | History shows this regressed repeatedly. Unreal/PIE automation still must prove the same against real client prediction and aggressive yaw. | Run PIE/client validation for the same scenarios and keep server guard tests as CI baseline. Fix common owner, not per-skill patches. |
...
- Shield Rush physical root motion is restored as a longer committed rush: `shield_rush_front_contact_v1` travels `470cm`, uses `620cm/s` base speed, and keeps target/control carry distance aligned with `skill.movement_distance`.
- Grounded forward-distance skills must not use target center or mouse ground position as movement anchors; target/mouse data remain hit/contact inputs while movement direction comes from horizontal facing/aim.
...
- Server tests must fail if Shield Rush falls back to the old short `340cm` envelope or if target/mouse anchors can redirect close-range forward movement.
...
- Creature setup windup and creature skill root motion publish different locomotion owners: setup uses grounded move, while lunge/maul movement contracts are emitted only during the root-motion window. Unreal must never receive a leap/maul movement contract for a grounded setup tick.
- The setup policy chosen at skill selection is carried across the active creature action, so lunge windup cannot flip between flank/chase policies after the instance starts.
...
- Recovery-only compatibility migrations are not part of the fresh dev baseline.
- Any compatibility field still required by an endpoint is present in its canonical `CREATE TABLE`, not added later by `ALTER`.
