...
| **Crítica** 🔨 | **Shield Rush/Bash reconciliáveis** — **= rubberband atual do R/F**. **FEITO:** `675c379` server agora publica `grounded_skill_action` (Unreal reconhece como `SkillGroundedAction`; antes mandava `..._reconciliation` → `None`) + regression test espelhando o parser do Unreal; `EntrySpeed`/distância via kinematics (`f96b667`). **Falta:** `command_id` no intent de skill + previsão local (`SkillDash`) no client; idem para creature bite/maul (menos crítico). | **chat 4** | gameapi (vivo) — **Fatia 1** |
...
2b. ⏳ **DB-authoritative damage (cross-project, BLOQUEIA o 3 "certo"):** CONFIRMADO — o **db-apeiron já tem tudo**: `proto/apeiron/v1/common.proto message Skill` com `base_damage`(2), `max_range`(8), `posture_damage`(16), `max_targets`(19), `movement_distance`(20), `is_blockable`(17) + seed + repository que lê. **O gargalo é o lado SERVER:** o contrato DB do server (`dbv1`) não inclui o `Skill` message, e `loadSkillRuntimeContract` só busca `GetSkillActionTiming`/`GetSkillMovementActionBinding` — nunca o dano/range. Trabalho: reconciliar o contrato DB do server (adicionar fetch do `Skill` base via `SkillDataService` no proto do server + client `dbapeiron.Skills` + **regerar gen** com `scripts/generate_proto.bat` — protoc/plugins disponíveis em `~/OneDrive/Documentos/bin` e `~/go/bin`), e mapear `base_damage→Damage`, `posture_damage→PostureDamage`, `max_range→Range` em `loadSkillRuntimeContract`. O tijolo 3 usa `max_range` (range) + direção; cone_angle não está no proto Skill (refino).
