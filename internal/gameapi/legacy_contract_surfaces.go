...
const (
	...
	contractSurfaceRecoveryOnly          = "recovery_only"
...
func runtimeContractSurfaces() []RuntimeContractSurface {
	return []RuntimeContractSurface{
		...
		},
		{
			...
			Decision:                "keep endpoint coherent for migration/recovery only; never use it to decide player or creature skill root motion",
		},
		{
		...
		},
		{
			...
			Status:                 contractSurfaceRecoveryOnly,
			NormalRuntimeAuthority: false,
			CanonicalReplacement:   "skill_hitbox_profile.motion_profile_id + skill_hitbox_motion_sample.motion_profile_id",
			Decision:               "recovered inverse-column tolerance only; runtime must use the canonical motion profile relation",
		},
		{
			Name:                   "skill_hitbox_motion_sample.id_text_legacy_shape",
			Status:                 contractSurfaceRecoveryOnly,
			NormalRuntimeAuthority: false,
		...
		{
			Name:                   "creature_orbit_policy.legacy_radius_columns",
			Status:                 contractSurfaceRecoveryOnly,
			NormalRuntimeAuthority: false,
		...
		},
		{
			...
			NormalRuntimeAuthority: false,
			CanonicalReplacement:   "current compact reconstructed migrations plus compatibility map",
			Decision:               "filename numbering alone is not runtime authority; do not recreate duplicate migrations just for old numbers",
		},
	}
}

func legacyRuntimeSurfaceBlockers() []string {
	var blockers []string
...
func legacyRuntimeSurfaceStatusValues() map[string]string {
